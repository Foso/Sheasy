...
class MainFragment : BaseFragment(), EventAdapter.OnTagClickListener, ITabView {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
       ...
       serverSwitch?.setOnCheckedChangeListener { buttonView, isChecked -> when(isChecked){
           true->{
               profileViewModel.startService( Intent(activity, HTTPServerService::class.java))
               App.instance.sendBroadcast(Event(EventCategory.SERVER, "started", LocalDateTime.now().toString()))
